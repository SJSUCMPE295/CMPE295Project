...
useEffect(() => {
    axios.get('/api/home',{ params: { user:'617f2fb40583ba49a0091425'} })//change later
      .then((response) => {
   ...
   const userappointments=data.userappointments;
    ...
    return (
        ...
        <Grid item lg={3} sm={6} xl={3} xs={12}>
         ...
         {data != null? <Typography color="textPrimary" variant="h3">{data.transactions}</Typography>: null } 
        ...
        <CardContent>
            <Grid container spacing={3} sx={{ justifyContent: 'space-between' }}>
                <Grid item>
                    ...
                    {data != null? <Typography color="textPrimary" variant="h3">{totalresources}</Typography>: null }
        ...
        <CardContent>
            <Grid container spacing={3} sx={{ justifyContent: 'space-between' }}>
                <Grid item>
                    ...
                    {data != null? <Typography color="textPrimary" variant="h3">{data.users}</Typography>: null }
                </Grid>
                <Grid item>
                    <Avatar
                        sx={{
        ...
        <CardContent>
            <Grid container spacing={3} sx={{ justifyContent: 'space-between' }}>
                <Grid item>
                    ...
                    {data != null? <Typography color="textPrimary" variant="h3">{data.services.length}</Typography>: null } 
                </Grid>
                <Grid item>
                    <Avatar
                        sx={{
        ...
        <PerfectScrollbar>
            <Box sx={{ minWidth: 800 }}>
                <Table>
                    ...
                    {data != null?<TableBody>{data.services.map((service) => ( <TableRow hover key={service._id}><TableCell>{service.Service_Name}</TableCell>
                                <TableCell>{service.Category}</TableCell>
                                <TableCell>
                                    {moment(service.availableDate).format('DD/MM/YYYY')}
                                </TableCell>
        ...
        >
            ...
            <Divider />
            <CardContent>
            <Typography variant="h6">Total Transactions : {totalusertrans}</Typography>
            {data && usertransactions.length>0?
                ...
                >   
                  ...
                  {data !=null ? userappointments.length>0 ? userappointments.map(({ DoctorId:DoctorId,AppointmentDetails:AppointmentDetails }) => (
                  <Typography color="textPrimary" variant="text">You have an appointment with {DoctorId} at {AppointmentDetails}</Typography>)): <Typography  color="textPrimary" variant="h7">You do not have any appointment scheduled.</Typography> :null} 
