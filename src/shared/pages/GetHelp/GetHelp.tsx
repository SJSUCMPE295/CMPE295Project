...
const GetHelp : FunctionComponent<any> = ({userProfileReducer,props }) => {
  const location = useLocation();
    ...
    const handlePageLoad= () => {
     ...
     axios
      .get('/api/gethelp', { params: { name: name , miles : miles, city :"",datafilter:datafilter,user_location:UserCompleteAddress,UserId:userProfileReducer?.id}}, {
        headers : {
            authtoken: token
        }
        }) 
      .then(
          (response) => {
            ignoreStat = true;
              ...
              setDirections(null);
              if(Object.keys(response?.data?.resources).length==0)
              {
                setDataError("No results matching your search criteria");
              }
              else{
                setDataError("");
              }
    ...
    const handleSearch= (event) => {
       ...
       if(miles!="" && city==""){
        ...
        axios
        .get('/api/gethelp', { params: { name: name , miles : miles, city :city,datafilter:datafilter,user_location:UserCompleteAddress,UserId:userProfileReducer?.id} }, {
          headers : {
              authtoken: token
          }
          }) 
...
<FormControlLabel value="services" control={<Radio />} label="Services" />
    ...
    />}
        ...
        <Typography   variant="body2" color="text.secondary">
            {(isNaN(resource.distance)?"0":resource.distance)+"miles"}</Typography>
        <Typography  variant="body2" color="text.secondary">
        {resource.address}
