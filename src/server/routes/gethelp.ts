...
router.get('/',async (_req, res) => {
    ...
    ,{   
        $project:{
                  ...
                  Description: 1,
                  SKU:1,
                  address :{ $concat: ["$addresses.Address.City",",","$addresses.Address.State",",","$addresses.Address.Country"] },
                  distance :""            
                }
...
const distance_matrix=  await client
    ...
    .then((r) => {
        ...
        {
            destArray[destinations[k]]=calculatemiles(r.data.rows[0].elements[0].distance.text)
