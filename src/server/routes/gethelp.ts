...
const getUserCurrentLocation = async () => {
    ...
    const user_loc = await client.geolocate({
        params: {
    ...
    const user_currentcity =
        (await current_city.data.results[0].address_components[3].short_name) +
        ', ' +
        (await current_city.data.results[0].address_components[5].short_name);
...
*/
    ...
    if ((await geocodedaddress.data.results.length) > 0) {
        const distance_matrix = await client.distancematrix({
            params: {
                ...
                for (var i = 0; i < destinations.length; i++) {
                    if (resources[j]['address'] == destinations[i]) {
                        resources[j]['distance'] = calculateMiles(
                            await distance_matrix.data.rows[0].elements[i].distance.text
                        );
                        resources[j]['location'] = await geocodedaddress.data.results[i].geometry
                            .location;
                    }
                }
            }
...
router.get('/', async (_req, res) => {
    ...
    if (type == 'pageload') {
        ...
        getUserCurrentLocation()
            .then((user_location) => {
                if (user_location.user_currentcity != '') {
                    city = user_location.user_currentcity.slice(
                        0,
                        user_location.user_currentcity.indexOf(',')
                    );
                    //console.log(city);
                ...
                getData(
                    query_params_resource,
                    query_params_service,
                    response.user_currentaddress,
                    miles,
                    datafilter
                )
                    .then((resources) => {
                        response.resources = resources; //console.log(response);
                        res.send(response);
                    })
                    .catch((e) => {
                        console.log(e);
        ...
        getData(query_params_resource, query_params_service, user_loc, miles, datafilter)
            .then((resources) => {
