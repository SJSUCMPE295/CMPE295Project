...
router.get('/', async (_req, res) => {
   ...
   if (name != '' && city=='' && miles==''){//name only
    query_params_resource = { Resource_Name: {'$regex': name.trim(),$options:'i'},UserId: {$ne: UserId} };
    query_params_service = { Service_Name: {'$regex': name.trim(),$options:'i'},UserId: {$ne: UserId}};
   }
   else if (name == '' && city!='' && miles=='') {//city only
    city=city.slice(0,city.indexOf(','));
        query_params_resource = { City:  {'$regex': city.trim(),$options:'i'} ,UserId: {$ne: UserId}};
        query_params_service = { City:  {'$regex': city.trim(),$options:'i'},UserId: {$ne: UserId} };
   ...
   /* else if (name == '' && city!='' && miles!='') {//city and miles
        ...
        city=city.slice(0,city.indexOf(','));
        query_params_resource = {  Resource_Name:  {'$regex': name.trim(),$options:'i'}, City:  {'$regex': city.trim(),$options:'i'},UserId: {$ne: UserId} };
        query_params_service = { Service_Name:  {'$regex': name.trim(),$options:'i'}, City: {'$regex': city.trim(),$options:'i'} ,UserId: {$ne: UserId}};
    }
